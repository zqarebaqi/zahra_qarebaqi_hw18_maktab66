...
function validate(values) {
  ...
  if (!values.password) {
    ...
    errors.password = "رمز عبور وارد شده خیلی کوتاه است";
  } else if (values.password.length > 50) {
    errors.password = "رمز عبور وارد شده خیلی طولانی است";
  }
  return errors;
}
...
const Register = () => {
  ...
  const handleCities = useCallback((province) =>
    setActiveCities(iranstates[province])
  );

  const handleGetCities = () => {
    axios
      .get("/db/iranstates.json")
  ...
  const formik = useFormik({
    ...
    onSubmit: (values, { setSubmitting, resetForm }) => {
      setSubmitting(false);
      axios
        .post("http://localhost:3001/users", { id: Date.now(), ...values })
        .then(() => {
      ...
      <div className="container-width_50">
        ...
        {formik.values.education.length > 0 ? (
          <div className="field width_50">
            ...
            />
            <p className="error">
              {formik.errors.eduPlace &&
                formik.touched.eduPlace &&
                formik.errors.eduPlace}
            </p>
          </div>
        ) : null}
